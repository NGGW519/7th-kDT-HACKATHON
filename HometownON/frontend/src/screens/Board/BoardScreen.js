...
import FreeBoardScreen from "./FreeBoardScreen.tsx";
...
export default function BoardScreen({ route }) {
  ...
  const handlePressWrite = () => {
    ...
    if (activeTab === "의뢰 게시판") {
      navigation.navigate("RequestBoardWriteScreen");   // 의뢰글 작성
    } else if (activeTab === "멘토 게시판") {
      navigation.navigate("MentorBoardWriteScreen");  // 멘토글 작성
    } else {
      navigation.navigate("FreeBoardWriteScreen", { boardType: "자유 게시판" });    // 자유글 작성
    }
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.contentPanel}>
        ...
        {activeTab === "자유 게시판" && <FreeBoardScreen navigation={navigation} route={route} />}
      </View>
    </SafeAreaView>
  );
}
